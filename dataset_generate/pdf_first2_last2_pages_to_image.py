"""
将 PDF「前 2 页 + 后 2 页」按「每页裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸」后 2x2 拼接。
输出尺寸固定为 2×Letter 宽 × 2×Letter 高（与 Letter 同比例）。
布局：左上=第1页、右上=第2页、左下=倒数第2页、右下=最后1页。

PDF 目录（输入）:
  /data/docpc_project/datasets/pdfa_test/pages_4_30/{cat}_pos_target_for_deepseek
输出: .../pdfa_test/{category}/pos_target_for_deepseek_images_first2_last2
"""
import os
import sys
...
FINAL_SUBDIR = "pos_target_for_deepseek_images_first2_last2"
CATEGORIES = [
    "biology",
    "education",
    "finance",
...
def pdf_first2_last2_to_image(
    pdf_path: str | Path,
    dpi: int = DPI,
) -> Image.Image:
    """
    将 PDF「前 2 页 + 后 2 页」按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸」后 2x2 拼接。
    布局：左上=第1页(0)、右上=第2页(1)、左下=倒数第2页(len-2)、右下=最后1页(len-1)。
    若页数<4，则用已有页重复填充（如 1 页则四格同页，2 页则上排 0,1 下排 1,0，3 页则 0,1,2,2）。
    """
...
def run_all(categories: list[str]) -> None:
    """
    按类别遍历 PDF：每份 PDF 前2页+后2页按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter」后 2x2 拼接，
    保存到 pos_target_for_deepseek_images_first2_last2；已存在的 PNG 跳过。
    """
    for cat in categories:
        ...
        for pdf_path in tqdm(pdf_files, desc=f"[{cat}] first2+last2 2x2"):
            out_path = out_dir / (pdf_path.stem + ".png")
            if out_path.exists():
                continue
            try:
                img = pdf_first2_last2_to_image(pdf_path, dpi=DPI)
...
def main():
    print("单页流程：裁白边 → 补白到 Letter 比例 → 缩放到 Letter → 前2页+后2页 2x2 拼接")

"""
将 PDF 前 4 页按「每页裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸」后 2x2 拼接。
输出尺寸固定为 2×Letter 宽 × 2×Letter 高（与 Letter 同比例）。

PDF 目录（输入）:
  /data/docpc_project/datasets/pdfa/pages_4_30/{cat}_pos_target_for_deepseek
输出: .../pdfa/{category}/pos_target_for_deepseek_images_first4
"""
import os
import sys
...
FINAL_SUBDIR = "pos_target_for_deepseek_images_first4"
CATEGORIES = [
    "biology",
    "education",
    "finance",
...
def pdf_first4_to_image(
    pdf_path: str | Path,
    dpi: int = DPI,
) -> Image.Image:
    """
    将 PDF 前 4 页按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸」后 2x2 拼接。
    输出尺寸固定为 (2*LETTER_W_PX, 2*LETTER_H_PX)，即 Letter 四宫格。
    """
    ...
    try:
        ...
        imgs = [process_one_page(doc, i, dpi) for i in range(4)]
        # 2x2：左上 0、右上 1、左下 2、右下 3；每张已是 LETTER_W_PX x LETTER_H_PX
...
def run_all(categories: list[str]) -> None:
    """
    按类别遍历 PDF：每份 PDF 前 4 页按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter」后 2x2 拼接，
    直接保存到 FINAL_SUBDIR；已存在的 PNG 跳过。
    """
    for cat in categories:
        ...
        for pdf_path in tqdm(pdf_files, desc=f"[{cat}] Letter 2x2"):
            out_path = out_dir / (pdf_path.stem + ".png")
            if out_path.exists():
                continue
            try:
                img = pdf_first4_to_image(pdf_path, dpi=DPI)
...
def main():
    print(f"单页流程：裁白边 → 补白到 Letter 比例 → 缩放到 {LETTER_W_PX}x{LETTER_H_PX} → 4 张 2x2 拼接")

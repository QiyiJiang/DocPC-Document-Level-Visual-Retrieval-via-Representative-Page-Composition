"""
将 PDF 第 1 页（裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸）保存为单张图。
不足 1 页则用空白。输出尺寸 = Letter 单页。

输入: PAGES_BASE / {cat}
输出: PROJECT_DIR / pdfa / {cat } / pos_target_for_deepseek_images_first1
"""
import os
import sys
...
FINAL_SUBDIR = "pos_target_for_deepseek_images_first1"
CATEGORIES = [
...
def pdf_first1_to_image(pdf_path: str | Path, dpi: int = DPI) -> Image.Image:
    """取第 1 页（不足则循环/空白），单页 Letter 输出。"""
...
def run_all(categories: list[str]) -> None:
    for cat in categories:
        ...
        for pdf_path in tqdm(pdf_files, desc=f"[{cat}] first1"):
            out_path = out_dir / (pdf_path.stem + ".png")
            if out_path.exists():
                continue
            try:
                img = pdf_first1_to_image(pdf_path, dpi=DPI)
...
def main():
    print(f"first1: 单页 Letter {LETTER_W_PX}x{LETTER_H_PX}")
    run_all(CATEGORIES)
    print("完成")

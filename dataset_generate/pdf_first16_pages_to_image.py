"""
将 PDF 前 16 页（不足则循环使用已有页）按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter」后 4×4 拼接。
输出尺寸 = 4×Letter 宽 × 4×Letter 高。

输入: PAGES_BASE / {cat}
输出: PROJECT_DIR / pdfa / {cat} / pos_target_for_deepseek_images_first16
"""
import os
import sys
...
FINAL_SUBDIR = "pos_target_for_deepseek_images_first16"
CATEGORIES = [
...
def pdf_first16_to_image(pdf_path: str | Path, dpi: int = DPI) -> Image.Image:
    """前 16 页，不足则循环使用前面页，4×4 拼接。"""
...
def run_all(categories: list[str]) -> None:
    for cat in categories:
        ...
        for pdf_path in tqdm(pdf_files, desc=f"[{cat}] first16 4x4"):
            out_path = out_dir / (pdf_path.stem + ".png")
            if out_path.exists():
                continue
            try:
                img = pdf_first16_to_image(pdf_path, dpi=DPI)
...
def main():
    print(f"first16: 16 页 4×4 拼接，不足 16 页循环使用前若干页")
    print(f"输出尺寸：{4 * LETTER_W_PX} x {4 * LETTER_H_PX}")
    run_all(CATEGORIES)
    print("完成")

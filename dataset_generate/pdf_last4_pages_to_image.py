"""
将 PDF「后 4 页」按「每页裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸」后 2x2 拼接。
布局：左上=倒数第4页、右上=倒数第3页、左下=倒数第2页、右下=最后1页。
输出尺寸固定为 2×Letter 宽 × 2×Letter 高（与 Letter 同比例）。

PDF 目录（输入）:
  /data/docpc_project/datasets/pdfa_test/pages_4_30/{cat}_pos_target_for_deepseek
输出: .../pdfa_test/{category}/pos_target_for_deepseek_images_last4
"""
import os
import sys
...
FINAL_SUBDIR = "pos_target_for_deepseek_images_last4"
CATEGORIES = [
    "biology",
    "education",
    "finance",
...
def pdf_last4_to_image(
    pdf_path: str | Path,
    dpi: int = DPI,
) -> Image.Image:
    """
    将 PDF 后 4 页（倒数第4、第3、第2、最后1页）按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter 尺寸」后 2x2 拼接。
    若页数 < 4，则用已有页重复填充（同 first4 的规则）。
    """
    ...
    try:
        ...
        # 四格：左上、右上、左下、右下 = 倒数第4、第3、第2、最后1页
        if n >= 4:
...
def run_all(categories: list[str]) -> None:
    """
    按类别遍历 PDF：每份 PDF 后 4 页按「裁白边 → 补白到 Letter 比例 → 缩放到 Letter」后 2x2 拼接，
    保存到 pos_target_for_deepseek_images_last4；已存在的 PNG 跳过。
    """
    for cat in categories:
        ...
        for pdf_path in tqdm(pdf_files, desc=f"[{cat}] last4 2x2"):
            out_path = out_dir / (pdf_path.stem + ".png")
            if out_path.exists():
                continue
            try:
                img = pdf_last4_to_image(pdf_path, dpi=DPI)
...
def main():
    print("单页流程：裁白边 → 补白到 Letter 比例 → 缩放到 Letter → 后 4 页 2x2 拼接")
    print(f"布局：左上=倒数第4页、右上=倒数第3页、左下=倒数第2页、右下=最后1页")

...
class ContrastiveTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 检查是否有positive_mask
        positive_mask = inputs.get("positive_mask", None)
        ...
        if "neg_doc_input_ids" in inputs:
            ...
            # 如果有positive_mask，尝试传递给损失函数
            if positive_mask is not None:
                try:
                    loss_result = self.loss_func(query_outputs, doc_outputs, positive_mask=positive_mask)
                except TypeError:
                    # 如果损失函数不支持positive_mask参数，使用传统方式
                    loss_result = self.loss_func(query_outputs, doc_outputs, neg_doc_outputs)
            else:
                loss_result = self.loss_func(query_outputs, doc_outputs, neg_doc_outputs)
        ...
        if self.accelerator.num_processes > 1 and self.accelerator.sync_gradients:
            ...
            # 同样需要gather positive_mask
            if positive_mask is not None:
                positive_mask = self.accelerator.pad_across_processes(positive_mask, dim=1, pad_index=False, pad_first=True)
                positive_mask = concat_all_gather(positive_mask)
            
            rank = self.accelerator.process_index
            offset = rank * num_items_in_batch

        # 传递positive_mask给损失函数
        if positive_mask is not None:
            try:
                loss_result = self.loss_func(query_outputs, doc_outputs, positive_mask=positive_mask, offset=offset)
            except TypeError:
                # 如果损失函数不支持positive_mask参数，回退到原始调用
                loss_result = self.loss_func(query_outputs, doc_outputs, offset=offset)
        else:
            loss_result = self.loss_func(query_outputs, doc_outputs, offset=offset)

        if isinstance(loss_result, tuple):
            loss = loss_result[0]  # 只取损失值
        else:
            loss = loss_result
    ...
    def prediction_step(self, model, inputs, prediction_loss_only, ignore_keys=True):
        ...
        with torch.no_grad():
            ...
            # 检查是否有positive_mask（多正例情况）
            if "positive_mask" in inputs:
                positive_mask = inputs["positive_mask"]
                try:
                    # 尝试使用positive_mask参数
                    loss_result = self.loss_func(query_outputs, doc_outputs, positive_mask=positive_mask)
                except TypeError:
                    # 如果损失函数不支持positive_mask参数，回退到原始调用
                    loss_result = self.loss_func(query_outputs, doc_outputs)
            else:
                # 单正例情况
                loss_result = self.loss_func(query_outputs, doc_outputs)

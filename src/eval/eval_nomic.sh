#!/usr/bin/env bash
# Nomic 嵌入模型评估脚本：多策略 × 7 类别
# Nomic 通过 API 调用，不需要 GPU。
#
# 使用 nohup 后台运行:
#   cd /data/docpc_project && nohup bash eval_nomic.sh > eval_nomic.log 2>&1 &
#   tail -f eval_nomic.log

set -e

# ==================== 参数配置 ====================
QUERY_ROOT="/data/docpc_project/dataset_generate_new/pdfa_test"
RESULTS_DIR="/data/docpc_project/500——eval_results/nomic"
...
for strategy in "${STRATEGIES[@]}"; do
    ...
    for cat in "${CATEGORIES[@]}"; do
        ...
        RUN_TAG="nomic__${strategy}__${cat}"
